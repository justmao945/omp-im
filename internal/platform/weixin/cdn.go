...
func aesECBPaddedSize(plaintextLen int) int {
	...
	return ((plaintextLen + aes.BlockSize - 1) / aes.BlockSize) * aes.BlockSize
}
...
func uploadBufferToCDN(ctx context.Context, client *http.Client, cdnURL string, plaintext, aesKey []byte, label string) (string, error) {
	cipher, err := encryptAESECB(plaintext, aesKey)
	if err != nil {
	...
	for attempt := 0; attempt < 3; attempt++ {
		...
		req, err := http.NewRequestWithContext(ctx, http.MethodPost, cdnURL, bytes.NewReader(cipher))
		if err != nil {
			return "", fmt.Errorf("%s: new request: %w", label, err)
		}
		req.Header.Set("Content-Type", "application/octet-stream")
		req.Header.Set("Content-Length", fmt.Sprintf("%d", len(cipher)))
		...
		body, err := io.ReadAll(io.LimitReader(resp.Body, maxIlinkHTTPResponseBody+1))
		resp.Body.Close()
		if err != nil {
			lastErr = err
			continue
		...
		if resp.StatusCode != http.StatusOK {
			lastErr = fmt.Errorf("http %d: %s", resp.StatusCode, truncateForLog(body, 256))
			continue
		}
		...
		if err := json.Unmarshal(body, &uploadResp); err == nil && strings.TrimSpace(uploadResp.DownloadParam) != "" {
			return uploadResp.DownloadParam, nil
		}
		// Some gateways return the download param as the plain response body.
		dl := strings.TrimSpace(string(body))
		if dl != "" {
			return dl, nil
		}
		lastErr = fmt.Errorf("empty download_param")
	}
	return "", fmt.Errorf("%s: CDN upload failed after 3 attempts: %w", label, lastErr)
}
